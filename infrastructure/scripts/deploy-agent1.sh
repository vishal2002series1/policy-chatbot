...
if [ -z "$KB_ID" ] || [ "$KB_ID" = "null" ]; then
    ...
    KB_CREATE_RESPONSE=$(aws bedrock-agent create-knowledge-base \
        ...
        --region "$REGION" \
        --knowledge-base-configuration "{\"embeddingModelArn\":\"$EMBEDDING_MODEL_ARN\"}" \
        --storage-configuration "{\"opensearchServerlessConfiguration\":{\"collectionArn\":\"$OPENSEARCH_COLLECTION_ARN\",\"vectorIndexName\":\"$VECTOR_INDEX_NAME\"}}" \
